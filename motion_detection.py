    ...
    while True:
        ...
        for c in cnt:
            if cv2.contourArea(c) > 800: # if contour area is less then 800 non-zero(not-black) pixels(white)
                ...
                cv2.rectangle(frame, (x,y), (x+w, y+h), (0, 0, 255), 2) # (0, 255, 0) = color R,G,B = lime / 2 = thickness(i think?)(YES IM RITE!)
        ...
        dilate_image = imutils.resize(dilate_image, width=800)
        frame_delta = imutils.resize(frame_delta, width=800)
